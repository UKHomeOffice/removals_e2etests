...
module.exports = {
  commands: [{
    ...
    toggleCentreDetailsCid: function (centreName, gender, k) {
      let centreDetailCidToggle = `${getCentreGenderScope(centreName, gender)}//td/text()[contains(., "${k}")]`
      this.api
        .useXpath()
        .expect.element(centreDetailCidToggle).to.be.present.after(1000)
      this.api
    ...
    expectCentreDetailCids: function (centreName, gender, k, cid) {
      this.api.useXpath()
      this.expect.element(`${getCentreGenderScope(centreName, gender)}//td/text()[contains(., "${k}")]`).text.to.contain(cid).before(2000)
      this.api.useCss()
    }
  }],
  elements: {
