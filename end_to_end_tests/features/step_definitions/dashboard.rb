...
Then(/^the number of available beds displayed is correct$/) do
  ...
  @new_post.each do |post|
    ...
    centre_male_beds = @dashboard_page.disp_male_available_beds(post.get_centre_num)
    centre_female_beds = @dashboard_page.disp_female_available_beds(post.get_centre_num)
...
And(/^number available beds have been reset to default$/) do
  @new_post.each do |post|
    disp_available_m_beds = @dashboard_page.disp_male_available_beds(post.get_centre_num)
    disp_available_f_beds = @dashboard_page.disp_female_available_beds(post.get_centre_num)
