...
And(/^occupancy has been reset to default$/) do
  @new_post.each do |post|
    disp_m_capacity = @stats_page.disp_male_capacity(post.centre_num)
    disp_f_capacity = @stats_page.disp_female_capacity(post.centre_num)
    disp_m_occupied_beds = @stats_page.disp_male_occupied(post.centre_num)
    disp_f_occupied_beds = @stats_page.disp_female_occupied(post.centre_num)
    disp_m_occ_beds = @stats_page.disp_male_occ(post.centre_num)
    disp_f_occ_beds = @stats_page.disp_female_occ(post.centre_num)
    disp_available_m_beds = @stats_page.disp_male_available_beds(post.centre_num)
    disp_available_f_beds = @stats_page.disp_female_available_beds(post.centre_num)
    ...
    if @dashboard_page.get_female_centre_capacity(post.centre_num) == 0
    else
...
And(/^number centre name is displayed correctly on the stats page$/) do
  @new_post.each do |post|
    disp_centre_name = @stats_page.disp_centre_name(post.centre_num)
